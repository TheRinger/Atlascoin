...
The RPM spec file provided here is for Atlas-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://atlas.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Atlas git repository](https://github.com/AtlasProject/Atlascoin) but are not part of
...
Currently only `Source10` is used, for the example `atlas.conf` file.
...
in the atlas git repository. At present only an SVG version of the Atlas
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the atlas
...
needed when 0.12.1 is released, a proper fix is already in the Atlas git
master branch.
...
idea to build Atlas Core against a static version of that library compiled
...
The `atlas.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `atlas-server` package. At this point, most RPM based
...
The `atlas-server` package should have SELinux support. How to properly do
...
Atlas-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/atlas/). He has been
packaging Atlas for Fedora at least since 2012.
...
4. I renamed the `atlas` package that contains the Qt GUI to `atlas-core` as
that appears to be how the general population refers to it, in contrast to
`atlas-xt` or `atlas-classic`. I wanted to make sure the general population
