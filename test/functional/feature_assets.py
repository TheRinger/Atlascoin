...
class AssetTest(AtlasTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 3

    def activate_assets(self):
        self.log.info("Generating ATL for node[0] and activating assets...")
    ...
    def big_test(self):
        ...
        n0.issue("ATLAS1", 1000)
        n0.issue("ATLAS2", 1000)
        n0.issue("ATLAS3", 1000)
        n0.generate(1)
        self.sync_all()

        n0.listassets(asset="ATLAS*", verbose=False, count=2, start=-2)
        ...
        atlas_assets = n0.listassets(asset="ATLAS*", verbose=False, count=2, start=-2)
        assert_equal(len(atlas_assets), 2)
        assert_equal(atlas_assets[0], "ATLAS2")
        assert_equal(atlas_assets[1], "ATLAS3")
        self.sync_all()
