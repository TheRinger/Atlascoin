...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests) {
        ...
        //- Versions of ATLASCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("ATL", type));
        BOOST_CHECK(!IsAssetNameValid("ATLAS", type));
        BOOST_CHECK(!IsAssetNameValid("ATLASCOIN", type));

        //- Versions of ATLASCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("ATLAS.COIN", type));
        BOOST_CHECK(IsAssetNameValid("ATLAS_COIN", type));
        BOOST_CHECK(IsAssetNameValid("ATLSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERATL", type));
        BOOST_CHECK(IsAssetNameValid("ATLASSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEATLAS", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_ATLASS", type));
        BOOST_CHECK(IsAssetNameValid("SEATLOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_check) {
        ...
        CAssetTransfer asset("ATLAS", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_check) {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("ATLAS", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
