...
void static AtlasMiner(const CChainParams& chainparams)
{
    LogPrintf("AtlasMiner -- started\n");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("AtlasMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("AtlasMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("AtlasMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("AtlasMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("AtlasMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateAtlass(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&AtlasMiner, boost::cref(chainparams)));
    }
