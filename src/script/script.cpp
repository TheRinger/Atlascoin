...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** ATL START */
    case OP_ATL_ASSET              : return "OP_ATL_ASSET";
    /** ATL END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_ATL_ASSET) { // OP_ATL_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == ATL_R) { // Check to see if ATL starts at 27 ( this->size() < 105)
                if ((*this)[28] == ATL_V)
                    if ((*this)[29] == ATL_N)
                        index = 30;
            } else {
                if ((*this)[28] == ATL_R) // Check to see if ATL starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == ATL_V)
                        if ((*this)[30] == ATL_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == ATL_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == ATL_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == ATL_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == ATL_R) {
